...
import { ShoppingCart, Eye, MoreHorizontal, Clock, Utensils, CheckCircle, XCircle, Send, CalendarIcon, Filter, ArrowUpDown, PlusCircle, ListOrdered, Hourglass } from 'lucide-react';
...
export default function OrderManagementPage() {
  ...
  const orderEditPanelClasses = cn(
    ...
    "w-full md:w-2/5" // Fixed width when visible
  );
  ...
  return (
    <div className="flex h-[calc(100vh-theme(spacing.16)-1px)] overflow-hidden relative"> {/* Ensure parent has fixed height */}
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button id="date" variant={"outline"} className={cn("w-full justify-start text-left font-normal h-10", !dateRange && "text-muted-foreground")} >
                          ...
                          {dateRange?.from ? (dateRange.to ? (<span>{format(dateRange.from, "LLL dd, y")} - {format(dateRange.to, "LLL dd, y")}</span>) : (<span>{format(dateRange.from, "LLL dd, y")}</span>)) : (<span>Pick a date range</span>)}
                        </Button>
                      </PopoverTrigger>
